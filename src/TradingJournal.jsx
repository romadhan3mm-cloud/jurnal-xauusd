...
import { Plus, Download, Upload, Trash2, ChevronDown, ChevronUp, MoreVertical, Archive, ArchiveRestore, X, Check, TrendingUp, TrendingDown, Lock, Sun, Moon, LogOut, LineChart as ChartIcon, BookOpen, CalendarClock, RefreshCw } from "lucide-react";
...
export default function TradingJournalApp({ userEmail, onLogout } = {}) {
  ...
  // Konfirmasi diklik — timpa data saat ini dengan isi file backup, lalu simpan ke Supabase
  const confirmImport = () => {
    if (!pendingImport) return;
    const { months: importedMonths, order: importedOrder } = pendingImport;
    setMonths(importedMonths);
    setOrder(importedOrder);
    setActiveMonth(importedOrder[importedOrder.length - 1] || null);
    persist(importedMonths, importedOrder);
      ...
      {pendingImport && (
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 12.5, color: "var(--red)", lineHeight: 1.5, margin: "0 0 16px" }}>
              Melanjutkan akan <strong>menimpa semua data jurnal yang ada sekarang</strong> (tabel, grafik, statistik) dengan isi file ini. Tindakan ini tidak bisa dibatalkan.
            </p>
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={confirmImport}
                style={{ flex: 1, background: "var(--red)", color: "#fff", border: "none", borderRadius: 7, padding: "9px 0", fontSize: 12.5, fontWeight: 600, cursor: "pointer" }}
              >
                Ya, Timpa & Import
              </button>
              <button
